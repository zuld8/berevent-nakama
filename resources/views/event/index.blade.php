    ...
    <main class="mx-auto max-w-7xl px-4 py-4">
        <section class="mb-3">
            @include('partials.header-user', [
                ...
                'query' => $q ?? '',
                'org' => $org ?? null,
                'searchRouteName' => 'event.index',
                'showFilterButton' => true,
            ])
        </section>

        <section class="mb-3">
            <x-categories :items="$categoryItems ?? []" :active="$activeCategory ?? null" :q="$q ?? null" title="Kategori" :seeAllHref="route('event.index')" routeName="event.index" />
        </section>
        ...
        <div id="event-filter-modal" class="fixed inset-0 z-50 hidden">
            ...
            <div class="absolute inset-x-0 bottom-0 rounded-t-2xl bg-white p-4 shadow-2xl">
                <div class="mx-auto max-w-2xl">
                    ...
                    <form method="get" action="{{ route('event.index') }}" class="grid grid-cols-1 gap-3 sm:grid-cols-3">
                        ...
                        <div class="flex items-end gap-2">
                            ...
                            <a href="{{ route('event.index', ['category' => $activeCategory, 'q' => $q]) }}" class="text-sm text-gray-600 hover:underline">Reset</a>
                        </div>
                    </form>
                </div>
            </div>
        ...
        @else
            <div class="flex items-center justify-between gap-4 mb-3">
                <h2 class="text-sm font-semi">Semua Event</h2>
            </div>
            <!-- 2-column event grid, menggunakan kartu vertikal -->
            <div id="event-grid" class="grid grid-cols-2 gap-3 mb-6">
                @foreach ($events as $e)
