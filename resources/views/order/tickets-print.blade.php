...
<head>
  ...
  <title>Cetak Tiket — {{ $order->reference }}</title>
  <style>
...
<body>
  <div class="page">
    <div class="header">
      <div>
        <div style="font-size:18px;font-weight:700;">Tiket Order {{ $order->reference }}</div>
    ...
    @else
      @foreach($tickets as $t)
        <div class="sheet">
          <div class="ticket">
            <div style="font-weight:700; font-size:18px;">{{ $t->event?->title ?? 'Event' }}</div>
            ...
            <div class="muted">Order: {{ $order->reference }}</div>
          </div>
        </div>
      @endforeach
    @endif
